...
export default function BlogCard({ post, readMoreLabel, byLabel, categoryLabels }: Props) {
  const title = post.title.no;
  const summary = post.summary.no;
  ...
  return (
    <Link href={`/blogg/${post.slug}`} className="group block bg-white rounded-xl shadow-sm border border-mist overflow-hidden hover:shadow-md transition-shadow">
      {imageUrl && (
