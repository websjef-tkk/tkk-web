...
export default async function KontaktPage() {
  ...
  return <KontaktContent people={people} />;
}

function KontaktContent({ people }: { people: SanityPerson[] }) {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
        <div className="bg-white rounded-xl border border-mist shadow-sm p-6">
          <h2 className="font-display font-bold text-navy text-xl mb-4">Adresse</h2>
          <div className="space-y-3 text-sm text-slate">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-teal mb-1">Besøksadresse</p>
              <p>Nedre Ila 12</p>
              <p>7018 Trondheim</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-teal mb-1">Postadresse</p>
              <p>Postboks 9167 Ilsvika</p>
              <p>7428 Trondheim</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-teal mb-1">Telefon</p>
        ...
        <div className="bg-white rounded-xl border border-mist shadow-sm p-6">
          <h2 className="font-display font-bold text-navy text-xl mb-4">Sosiale medier</h2>
      ...
      {board.length > 0 && <ContactTable title="Styre" people={board} />}
      {leaders.length > 0 && <ContactTable title="Gruppeledere" people={leaders} />}
      {others.length > 0 && <ContactTable title="Andre" people={others} />}

      <section className="mt-4">
        <h2 className="font-display font-bold text-navy text-2xl mb-4">Finn oss</h2>
...
function ContactTable({ title, people }: { title: string; people: SanityPerson[] }) {
  return (
