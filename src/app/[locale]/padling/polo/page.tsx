...
export default async function PoloPage({ params }: PageProps) {
  const { locale } = await params;
  return <PoloContent locale={locale} />;
}

function PoloContent({ locale }: { locale: string }) {
  const t = useTranslations("polo");

  const sections = [
    { title: t("about_title"), text: t("about_text") },
    { title: t("training_title"), text: t("training_text") },
    { title: t("competition_title"), text: t("competition_text") },
    { title: t("join_title"), text: t("join_text") },
  ];

  return (
    <article>
      <div className="relative h-64 md:h-96 overflow-hidden">
        <Image src="/images/hero.jpeg" alt="Kajakkpolo" fill className="object-cover" priority />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-mist rounded-xl p-5 mb-10 flex items-center gap-3">
          ...
          <a
            href="https://www.facebook.com/groups/kajakkpolo"
            target="_blank"
