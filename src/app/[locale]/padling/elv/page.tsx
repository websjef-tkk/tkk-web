...
export default async function ElvPage({ params }: PageProps) {
  ...
  return <ElvContent locale={locale} />;
}

function ElvContent({ locale }: { locale: string }) {
  const t = useTranslations("elv");

  const sections = [
    { title: t("local_title"), text: t("local_text") },
    { title: t("trips_title"), text: t("trips_text") },
    { title: t("winter_title"), text: t("winter_text") },
    { title: t("slalom_title"), text: t("slalom_text") },
  ...
  return (
    <article>
      <div className="relative h-64 md:h-96 overflow-hidden">
        <Image src="/images/hero.jpeg" alt="Elvepadling" fill className="object-cover" priority />
